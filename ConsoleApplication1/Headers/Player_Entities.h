...
Entities createShitCom()
{
	Entities pc;
	pc.role = "Shitcom";
	pc.health = 5000;       pc.maxHealth = pc.health;
	pc.armor = 600;         pc.magicRes = 1;
...
Entities createSoldier() {
	...
	case '2':
		...
		ar = 650;       mr = 500;
		rng = 2;        ap = 0;
		soldier.talent = "Spiky: Whenever got attacked, reflects 120 (+60% armor) (+60% magic resist) magic damage to the attacker. This damage also benefits from runes\n               Moral Support: If this unit starts the battle in teamfight mode, max HP +15%";
		soldier.spell_1 = "Reinforcement (3 mana): Immediately restore 200 + 20% missing health";
		soldier.spell_2 = "Form-up (3 mana): Briefly increases self resistance by 80%, then 'Taunt' and 'Silent' all enemies in 1 turn. 'Taunted' enemies will prioritize attack this unit, regardless of their current state or trait";
		break;
	case '3':
	...
	case '4':
		...
		soldier.spell_1 = "Volcano: Consume all mana, then attacks the enemy with 200% AP. Each mana spent increases the attack's firepower by 80 + 45%. Deals magic damage, 30% of the total damage is treated as pure damage";
		soldier.spell_2 = "Meteorite (3 mana): Deals 270% AP magic damage to all enemies, damage output is calculated based on the enemy with LEAST magic resist. This damage can not be blocked nor grazed, and can not trigger other effects";
		break;
	case '5':
		...
		hp = 3333;     ad = 400;      soldier.emergencyOn = true;
		ap = 0;        ar = 100;
		mr = 100;       rng = 5;
	...
	case '7':
		...
		soldier.spell_1 = "Body Slam (2 mana): Aggressively charges forward, deals 250 + 150% AP magic damage. Against target with 'Bleeding' status, damage is tripled and damage from 'Bleeding' increased to 20% max health";
		soldier.spell_2 = "Haemorrhage (1 mana): Affects the enemy with 'Bleeding'. The next source of direct damage they receive triggers this status, dealing 10% max health + 100% AP as magic damage.";
		break;
	case '8':
	...
	case 'A': case 'a':
		...
		ap = 350;       ar = 210;
		mr = 350;       rng = 10;
		soldier.talent = "Mechanical Egineer (Unique): Can rapidly manufactures and commands 'Machine' in combat (up to 3).\n               Machine: Costs 1 structure point to deploy and can attack enemy (starts with 3 and gains 1 point every 5 turns). Has 1500 HP, 350 attack damage, 300 armor, 100 magic resist and more likely to be attacked.\n               ";
		soldier.talent += "Sneak Shot: Whenever a friendly unit uses their first skill (skill must be offensive type), all Machines attack the target one additional time\n               Iron Will: When there is atleast 1 'Machine', normal attack instead heals the machine with least HP for 100% AP. Afterwards, this unit can select a target and commands all machines to attack them.";
	...
	case 'B': case 'b':
		...
		ap = 300;        ar = 150;
		mr = 150;        rng = 11;
		soldier.talent = "Destinated Path (Unique): Can not attack at will, and normal attack instead restores HP of ally equals 100% AP and grants to them 1 mana, self is less likely to be attacked.";
